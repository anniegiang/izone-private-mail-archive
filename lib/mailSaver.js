...
class MailSaver {
  ...
  async saveMail(mail, mailPath, imagesPath) {
    ...
    fs.writeFile(
      ...
      'utf8',
      function (error) {
        error && console.log(`❗️ Failed to save mail for ${mail.id}: `, error);
      }
    );
  }
  ...
  fileName(mail, mimeType = 'html', key = '') {
    ...
    const newMailSubject = mail.subject_ko.replace(/\//g, '-');
    return `${mail.id}__${mailDate}__${newMailSubject}__${
      key && `${key}__`
    }.${mimeType}`;
  }
  ...
  directoryExistsAsync(directory, cb) {
    return fs.access(directory, (error) => cb(error));
  }
