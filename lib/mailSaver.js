...
class MailSaver {
  ...
  async saveMail(mail, mailPath, imagesPath, cb) {
    ...
    const headerHTMLString = `<div id="mail-header"><div style="display:flex; justify-content: space-between; padding: 20px 15px; border-bottom: 1px solid Gainsboro; cursor: pointer;"><div style="display:flex;"><img data-id="member-image"src=${member.image_url} style="width: 3rem; margin-right: 10px; border-radius: 50%;" /><div><h4 style="margin: 0;">${member.realname_ko}</h4><h5  style="font-weight: lighter;  margin: 0;">To: <span style="color:gray;">${this.user.nickname}</span></h5></div></div><h4 style="color: silver; font-weight: lighter; margin:0;">${mail.receive_datetime}</h4></div><div style="padding: 20px 15px; border-bottom: 1px solid Gainsboro;"><h3 style="margin: 0;">${mail.subject_ko}</h3></div></div>`;
